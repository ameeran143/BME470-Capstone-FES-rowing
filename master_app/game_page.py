...
class SharedStats:
    def __init__(self):
        ...
        # for calibration - voltage-based (2V = back/0%, 6V = front/100%)
        self.back_voltage = 2.0  # Back position voltage (0%)
        self.front_voltage = 6.0  # Front position voltage (100%)
        self.fes_active_pos = None  # Will be calculated based on voltage
        self.seat_direction = 0  # Will be removed, kept for compatibility
        self.converted_fes_pos = None
        self.userID = None
        self.age = 0
        self.height = 0
        self.weight = 0
        ...
        # CSV playback for simulation mode
        self.csv_data = None  # List of dictionaries with CSV data
        self.csv_index = 0  # Current index in CSV data
        self.csv_playback_start_time = None  # When playback started
    ...
    def convert_raw_to_scale(self, voltage):
        """Convert voltage (2V-6V) to 0-100% scale"""
        if voltage is not None:
            # Clamp voltage to calibration range
            voltage = max(self.back_voltage, min(self.front_voltage, voltage))
            # Convert: 2V = 0%, 6V = 100%
            converted = ((voltage - self.back_voltage) / (self.front_voltage - self.back_voltage)) * 100
            return converted
        return None
    
    def convert_scale_to_raw(self, converted):
        """Convert 0-100% scale back to voltage (2V-6V)"""
        if converted is not None:
            # Convert: 0% = 2V, 100% = 6V
            voltage = self.back_voltage + (converted / 100.0) * (self.front_voltage - self.back_voltage)
            return voltage
        return None
    
    def load_csv_data(self, csv_file_path):
        """Load CSV sensor data for playback"""
        try:
            self.csv_data = []
            with open(csv_file_path, 'r') as file:
                ...
                for row in reader:
                    self.csv_data.append({
                        ...
                        'left_foot': float(row['Left Foot (ai16)']),
                        'right_foot': float(row['Right Foot (ai18)']),
                        'handle_force': float(row['Handle Force (ai20)']),
                        'handle_position': float(row['Handle Position (ai21)']),
                        'seat_position': float(row['Seat Position (ai22)'])  # Voltage
                    })
            self.csv_index = 0
            self.csv_loaded = True
            print(f"✅ Loaded {len(self.csv_data)} data points from CSV")
            return True
    ...
    def start_csv_playback(self):
        """Start CSV playback - call when entering simulation mode"""
        if self.csv_loaded and self.csv_data:
            self.csv_playback_start_time = time.time()
            self.csv_index = 0
            print("▶️  Starting CSV playback")
    
    def get_csv_sample_at_time(self, elapsed_time):
        """Get CSV sample at given elapsed time (in seconds)"""
        if not self.csv_loaded or not self.csv_data:
            return None
        
        # CSV times are relative (start at 0), so elapsed_time matches CSV time
        target_time = elapsed_time
        
        # If we've gone past the end, return None (no looping)
        if self.csv_index >= len(self.csv_data):
            return None
        
        # Find the sample at or just before target_time
        # Start from current index and move forward
        while self.csv_index < len(self.csv_data) - 1:
            if self.csv_data[self.csv_index + 1]['time'] > target_time:
                break
            self.csv_index += 1
        
        # If we've passed the end, return None
        if self.csv_index >= len(self.csv_data):
            return None
        
        return self.csv_data[self.csv_index]
    ...
    def update_stats(self):
        ...
        if not self.is_mac and self.hardware_mode:
            try:
                ...
                # Store voltage directly (not multiplied by 100)
                seat_voltage = seat_position  # Voltage from sensor (ai22)
                
                self.raw_seat_pos.append(seat_voltage)  # Store voltage
                ...
                # Convert voltage to 0-100% scale for display
                converted_pos = self.convert_raw_to_scale(seat_voltage)
                if converted_pos is not None:
                    self.converted_seat_position.append(converted_pos)
        ...
        # Simulation mode - CSV playback (always used on macOS, fallback for Windows/Linux)
        if not self.hardware_connected:
            # Load CSV data if not already loaded
            if not self.csv_loaded:
                csv_path = os.path.join(os.path.dirname(__file__), "Test_Recordings", "grace", "sensor_data.csv")
                if not self.load_csv_data(csv_path):
                    print("⚠️  Could not load CSV data, simulation disabled")
                    return
            
            # Start playback if not started
            if self.csv_playback_start_time is None:
                self.start_csv_playback()
            
            # Calculate elapsed time since playback started
            elapsed_time = time.time() - self.csv_playback_start_time
            
            # Get sample from CSV at current time
            sample = self.get_csv_sample_at_time(elapsed_time)
            
            if sample is None:
                # Reached end of CSV data (no looping)
                return
            
            # Extract seat position voltage from CSV
            seat_voltage = sample['seat_position']
            
            # Store voltage in raw_seat_pos
            self.raw_seat_pos.append(seat_voltage)
            
            # Convert voltage to 0-100% scale
            converted_pos = self.convert_raw_to_scale(seat_voltage)
            if converted_pos is not None:
                self.converted_seat_position.append(converted_pos)
            
            # Store other sensor data (for future use)
            self.handle_position.append(sample['handle_position'])
            self.handle_force.append(sample['handle_force'])
            self.L_foot_force.append(sample['left_foot'])
            self.R_foot_force.append(sample['right_foot'])
            
            # Track time for power calculations
            if not hasattr(self, 'temp_time'):
                self.temp_time = []
            self.temp_time.append(time.time())
            
            # Calculate power if we have enough data
            if len(self.raw_seat_pos) > 1 and len(self.temp_time) > 1:
                time_delta = self.temp_time[-1] - self.temp_time[-2]
                if time_delta > 0:
                    power = ((self.handle_force[-1] + self.handle_force[-2]) / 2) * \
                            abs(self.handle_position[-1] - self.handle_position[-2]) / time_delta
                    self.temp_power.append(power)
                    self.avg_power.append(sum(self.temp_power) / len(self.temp_power))
        
        # Update stroke rate (works for both hardware and CSV playback)
        if self.raw_seat_pos and len(self.converted_seat_position) >= 2:
            # Detect stroke completion (moving from front back to back)
            current_pos = self.converted_seat_position[-1]
            prev_pos = self.converted_seat_position[-2]
            # Stroke completes when moving from high position (>80%) back down
            if prev_pos > 80 and current_pos < prev_pos:
                ...
                if len(self.stroke_time) > 1:
                    ...
                    if self.stroke_duration[-1] > 0:
                        self.stroke_rate.append(60 / self.stroke_duration[-1])

        # Update score and misses (if switch_press data exists)
        if len(self.switch_press) > 1:
            if self.switch_press[-1] == 5 and self.switch_press[-2] == 0:  # button needs to be held pressed -> 5V
                ...
                # Note: FES timing logic would need to be updated for voltage-based system
                if self.is_pressed:
                    self.score += 1
                else:
                    self.misses += 1
    ...
    def stop_writing_stats(self):
        ...
        if self.sensor_task is not None:
            ...
            self.sensor_task = None
        # Reset CSV playback
        self.csv_playback_start_time = None
        self.csv_index = 0
...
class SpriteManager:
    ...
    def load_palm_tree_sprite(self, scale=1.0):
        """Load palm tree sprite from PNG file"""
        try:
            # Get the path to the palm_tree.png file (in assets folder)
            script_dir = os.path.dirname(__file__)
            palm_tree_path = os.path.join(script_dir, "assets", "palm_tree.png")
    ...
    def load_cloud_sprite(self):
        """Load cloud sprite from PNG file"""
        try:
            # Get the path to the cloud.png file (in assets folder)
...
class GamePage(wx.Panel):
    ...
    def on_timer(self, event):
        ...
        if self.shared_state.raw_seat_pos:
            print(f'Seat voltage: {self.shared_state.raw_seat_pos[-1]:.3f}V, Position: {self.shared_state.converted_seat_position[-1] if self.shared_state.converted_seat_position else 0:.1f}%')
        
        # Calculate distance (works for both hardware and CSV playback)
    ...
    def reset_game(self):
        # Reset CSV playback for new game
        self.shared_state.csv_playback_start_time = None
        self.shared_state.csv_index = 0
